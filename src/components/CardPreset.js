...
class CardPreset {
  constructor(gameObject) {
    ...
    this.gameObjectPreset.on("pointerdown", (pointer) => {
      ...
      if (this.gameObject.scene.oGameManager.isOwnTurn == true) {
        if (
          this.gameObjectPreset.x >= 350 &&
          this.gameObjectPreset.x <= 500 &&
          this.gameObjectPreset.y >= 750 &&
          this.gameObjectPreset.y <= 1000
        ) {
          ...
          if (this.gameObject.scene.oGameManager.isGrabCard == false) {
            ...
            for (
              var i = 0;
              i < this.gameObject.scene.discardDeckContainer.list.length;
              i++
            ) {
              this.gameObject.scene.discardDeckContainer.list[
                i
              ].disableInteractive();
            }
          }
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            console.log(
              "this.lastPosX",
              this.gameObjectPreset.x,
              "this.lastPosY",
              this.gameObjectPreset.y
            );
            this.gameObjectPreset.setPosition(
        ...
        //Group One
          ...
          this.gameObjectPreset.x >= 60 &&
          this.gameObjectPreset.x <= 515 &&
          this.gameObjectPreset.y >= 1105 &&
          this.gameObjectPreset.y <= 1305
        ) {
          ...
          this.lastHitcards =
            this.phaseOneHitCardIds[this.phaseOneHitCardIds.length - 1];
          if (this.gameObject.scene.isDeclarePhase == true) {
        ...
        //Group Two
          ...
          this.gameObjectPreset.x >= 565 &&
          this.gameObjectPreset.x <= 1020 &&
          this.gameObjectPreset.y >= 1105 &&
          this.gameObjectPreset.y <= 1305
        ) {
          ...
          this.lastHitcards1 =
            this.phaseTwoHitCardIds[this.phaseTwoHitCardIds.length - 1];
          if (this.gameObject.scene.isDeclarePhase == true) {
            if (this.gameObject.scene.oGameManager.phaseTwoType == "SET") {
              ...
              this.gameObject.scene.oGameManager.phaseTwoType == "RUN"
            ) {
              this.runContition(
                this.gameObject.scene.doublePhaseTwoCardContainer
              );
        ...
        // skip player
          ...
          this.gameObjectPreset.x >= 346 &&
          this.gameObjectPreset.x <= 734 &&
          this.gameObjectPreset.y >= 74 &&
          this.gameObjectPreset.y <= 697
        ) {
  ...
  handleParentContainerOperations(parentContainerName, gameObjectPreset) {
    switch (parentContainerName) {
      ...
      case "doublePhaseOneCardContainer":
        this.gameObject.scene.doublePhaseOneCardContainer.remove(
          gameObjectPreset
        );
        break;

      case "doublePhaseTwoCardContainer":
        this.gameObject.scene.doublePhaseTwoCardContainer.remove(
          gameObjectPreset
        );
        break;
    ...
    for (let i = 0; i < opponentContainer1.length; i++) {
      if (
        this.currentOwnCardLabel ===
          opponentContainer1[i].__CardPreset.cardNumber ||
        this.currentOwnCardLabel === "w"
      ) {
  ...
  setContition(groupContainer, phaseHitCardIds, lastHitcard, grp) {
    if (
      this.currentOwnCardLabel ===
        groupContainer.list[0].__CardPreset.cardNumber ||
      this.currentOwnCardLabel === "w"
    ) {
  ...
  runContition(groupContainer, phaseHitCardIds, lastHitcard, grp) {
    ...
    if (
      min - 1 == this.currentOwnCardLabel ||
      max + 1 == this.currentOwnCardLabel
    ) {
      groupContainer.add(this.gameObjectPreset);
      this.gameObjectPreset.disableInteractive();
      this.gameObjectPreset.setScale(0.6);
